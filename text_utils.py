...
class RenderFont(object):
            ...
            for ch in l: # render each character
                if ch.isspace(): # just shift
                    ...
                    try:
                        ...
                        return None,None,None  # considering the font which doesn't support rotation. add by ruifeng
    ...
    def render_curved(self, font, word_text):
        ...
        try:
            if vertical_FLAG:
                ch_bounds = font.render_to(surf, rect, word_text[mid_idx], rotation=rots[mid_idx]+90)
            else:
        ...
        for i in range(wl):
            ...
            try:
                if vertical_FLAG:
                    bbrect = font.render_to(surf, newrect, ch, rotation=rots[i]+90)
                else:
                    bbrect = font.render_to(surf, newrect, ch, rotation=rots[i])
            except ValueError:
                if vertical_FLAG:
                    bbrect = font.render_to(surf, newrect, ch, rotation=90)
                else:
...
class TextSource(object):
    ...
    def is_good(self, txt, f=0.35):
        ...
        """
        def is_txt(l):
            char_ex = ['i','I','o','O','0','-',' ','　']   # ' ' and '　' added by ruifeng for chinese lines.
    ...
    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):
        ...
        while not np.all(self.is_good(lines,f)) and iter < niter:
            ...
            for i in range(nline):
                ...
                # code for chinese use added by ruifeng
                characters = list(lines[i])
                dw = len(characters) - nword[i]
                if dw > 0:
                    first_c_index = random.choice(range(dw+1))
                    lines[i] = ''.join(characters[first_c_index:first_c_index+nword[i]])

                if len(lines[i]) > nchar_max:
                    lines[i] = lines[i][:nchar_max]

                '''
                # original code for english use edited by ruifeng
                words = lines[i].split()
                ...
                while len(lines[i]) > nchar_max: #chop-off characters from end:
                    print('length of length is: ',len(lines[i]))
                    print(lines[i])
